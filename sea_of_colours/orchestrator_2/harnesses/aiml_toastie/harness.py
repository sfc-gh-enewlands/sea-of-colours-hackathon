"""aiml_toastie — night entry point (clean fork of v11; changes land in Section 2).
...
both on the inference API) is native and on by default; ``AIML_TOASTIE_SINGLE=1``
...
from sea_of_colours.orchestrator_2.harnesses.aiml_toastie._v7 import (
    chat_schema,
...
from sea_of_colours.orchestrator_2.harnesses.aiml_toastie import prompt as prompt_mod
...
# ── Agent identity ─────────────────────────────────────────────────────
INNER_AGENT_LABEL = "AIML_TOASTIE"
THINKER_AGENT_LABEL = "AIML_TOASTIE_THINKER"
...
def _split_on() -> bool:
    """v12 runs the contained split by default; ``AIML_TOASTIE_SINGLE=1`` disables."""
    return os.environ.get("AIML_TOASTIE_SINGLE", "0").strip().lower() not in (
        "1", "true", "yes",
    )
...
def _autofill_on() -> bool:
    ...
    AGENT spends the shortfall and stays the author. Set ``AIML_TOASTIE_AUTOFILL=1``
    to restore the old guarantee and measure the two against each other.
    """
    return os.environ.get("AIML_TOASTIE_AUTOFILL", "0").strip().lower() in (
        "1", "true", "yes", "on",
    )
...
def run(
    *, store: Any, session_id: str, player: str, view: Mapping[str, Any],
    ...
    purchase_evidence = counter_chaff.prepare(
        agent_view, prior_day_entry, day=day, session_id=session_id, viewer=player,
    )
    ...
    counter_chaff.prepare_discovery(agent_view, last_night_memory, viewer=player)
    ...
    entry[counter_chaff.EVIDENCE_KEY] = purchase_evidence
